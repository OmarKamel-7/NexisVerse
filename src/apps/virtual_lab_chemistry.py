...
def run_chemistry_lab():
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            if dist<PINCH_THRESHOLD: pinch=True
        ...
        if finger_x and finger_y:
            for i,(x1,y1,x2,y2) in enumerate(boxes):
                if x1<=finger_x<=x2 and y1<=finger_y<=y2:
                    ...
                    if pinch:
                        choice=items[i]
                        if choice=="Back": cap.release(); cv2.destroyAllWindows(); return
                        elif choice=="Chemical Reactions": run_reactions()
                        elif choice=="Periodic Table": run_periodic_table()
                        elif choice=="Reaction Balancer": run_reaction_balancer()
